  ...
  async fetch(
    ...
    try {
      ...
      if (path === "/health") {
        ...
        response = await handleJobsProxyEndpoint({
          ctx,
          env,
          path,
          request,
          scheduleJobEvent: (auth, message) =>
...
function scheduleJobEvent(
  ...
  if (auth.session_id) {
    filter.session_ids = [auth.session_id];
  }
  if (auth.user_id) {
    filter.user_ids = [auth.user_id];
  }
  if (auth.tenant_id) {
    filter.tenant_ids = [auth.tenant_id];
  }
...
async function handleQueryStream(
  ...
  if (auth.session_key) {
    url.searchParams.set("session_key", auth.session_key);
  }
  if (auth.user_id) {
  ...
  return doStub.fetch(
    new Request(
      `https://internal/query_stream?${url.searchParams.toString()}`,
      request
    )
  );
}
 ...
 */
async function handleSessionEndpoint(
  request: Request,
  env: Env,
  path: string
): Promise<Response> {
  // Extract session ID from path
  const match = path.match(SESSION_PATH_REGEX);
  if (!match) {
  ...
  if (auth.tenant_id) {
    forwardUrl.searchParams.set("tenant_id", auth.tenant_id);
  }
 ...
 */
async function handleEventBusConnection(
  request: Request,
  env: Env
): Promise<Response> {
  const url = new URL(request.url);
