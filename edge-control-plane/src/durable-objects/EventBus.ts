...
export class EventBus extends DurableObject<Env> {
  ...
  private async loadConnectionInfo(): Promise<void> {
    const stored = await this.ctx.storage.get<Record<string, ConnectionInfo>>("connections");
    if (stored) {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      switch (path) {
        ...
        case "/connections":
          return this.handleGetConnections(request);
        case "/presence":
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    try {
      ...
      if (msg.type === "subscribe_session") {
        const sessionId = (msg.data as any).session_id;
  ...
  private async handleBroadcast(request: Request): Promise<Response> {
    ...
    for (const [connId, ws] of this.connections) {
      ...
      // Apply filters
      if (filter) {
        if (filter.session_ids && !filter.session_ids.some(sid => info.session_ids.includes(sid))) {
          continue;
        }
        if (filter.user_ids && (!info.user_id || !filter.user_ids.includes(info.user_id))) {
          continue;
        }
        if (filter.tenant_ids && (!info.tenant_id || !filter.tenant_ids.includes(info.tenant_id))) {
          continue;
        }
      }
  ...
  private handleGetPresence(request: Request): Response {
    ...
    // Group by user
    const userPresence: Record<string, { 
      user_id: string; 
