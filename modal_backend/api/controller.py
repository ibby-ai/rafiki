...
This file is started inside the sandbox via `uvicorn modal_backend.api.controller:app`
(see `modal_backend.main.get_or_start_background_sandbox`). The sandbox is created with an
encrypted port (8001), and `modal_backend.main.http_app` proxies to these endpoints.

See Modal docs for details about `modal.Sandbox`, encrypted ports, and tunnel
discovery.

Important:
- To reach this service from outside (via `modal_backend.main.http_app`), make sure the
  app is running with `modal serve -m modal_backend.main` (dev) or has been deployed with
...
from modal_backend.mcp_tools.session_tools import set_parent_context
