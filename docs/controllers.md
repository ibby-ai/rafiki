...
This document explains the role and implementation of the controller modules that serve as long-lived workers for the agent sandbox system.

## Overview

The system uses **two controller services** running in separate sandboxes:
...
| **CLI Controller** | `agent_sandbox/controllers/cli_controller.py` | 8002 | Claude Code CLI and Ralph loops |

Both controllers are **FastAPI microservices** running inside long-lived `modal.Sandbox` instances.

---

# Agent SDK Controller (controller.py)

## Overview

The Agent SDK controller is responsible for:
...
# CLI Controller (cli_controller.py)

## Overview

The CLI controller is responsible for:

1. **Executing Claude Code CLI** as subprocess
2. **Running Ralph autonomous coding loops**
3. **Managing CLI workspace** and job artifacts
4. **Non-root execution** via the `claude` user

## Location and Structure

**File:** `agent_sandbox/controllers/cli_controller.py`
...
- Volume management (`_maybe_reload_cli_volume`, `_commit_cli_volume`)

## How It's Started

The CLI controller is started inside a Modal Sandbox via uvicorn:
...
CLI_SANDBOX = modal.Sandbox.create(
    ...
    "--host", "0.0.0.0",
    "--port", str(CLI_SERVICE_PORT),  # Default: 8002
    name=CLI_SANDBOX_NAME,  # "claude-cli-runner"
    volumes={_settings.claude_cli_fs_root: cli_vol},  # /data-cli
    ...
)
```
...
1. Modal creates a sandbox environment
2. Runs `uvicorn agent_sandbox.controllers.cli_controller:app --host 0.0.0.0 --port 8002`
3. Uvicorn loads the FastAPI app from `cli_controller.py`
4. App listens on port 8002 (encrypted via Modal tunnel)
5. `http_app` discovers the tunnel URL and proxies requests
...
**Purpose:** Execute Claude Code CLI
...
1. Creates job workspace at `/data-cli/jobs/{job_id}/`
2. Builds CLI command with options
3. Runs CLI as `claude` user via `demote_to_claude()`
4. Captures stdout/stderr
5. Optionally writes result to file
6. Commits volume changes

### POST /ralph/execute

**Purpose:** Execute Ralph autonomous coding loop
...
1. Creates workspace at `/data-cli/jobs/{job_id}/`
2. Initializes git repository (if `auto_commit=true`)
3. Iterates through PRD tasks, calling Claude CLI for each
4. Runs feedback commands to validate work
5. Writes `status.json`, `progress.txt`, `prd.json`
6. Creates git commits after successful iterations
7. Returns final result

## Key Differences from Agent SDK Controller

| Aspect | Agent SDK Controller | CLI Controller |
|--------|---------------------|----------------|
| Port | 8001 | 8002 |
| User | root | claude (non-root) |
| Volume | `/data` | `/data-cli` |
| Purpose | Conversational queries | Code execution |
| Execution | Claude Agent SDK | Claude Code CLI subprocess |
| Sessions | Session resumption | Job workspaces |

## Non-Root Execution

The CLI controller runs Claude Code CLI as a non-root user:

```python
def demote_to_claude():
    """Return a function that sets process to claude user."""
    def result():
        os.setgid(CLAUDE_CLI_GID)
        os.setuid(CLAUDE_CLI_UID)
    return result
...
**Why non-root?**
- Claude Code CLI's `--dangerously-skip-permissions` flag requires non-root
- Prevents accidental root-level file operations
...
**Symptoms:** `/execute` returns errors

**Debug steps:**

1. Check that `ANTHROPIC_API_KEY` is available in the sandbox
2. Verify the `claude` user exists and has correct permissions
3. Check `/data-cli` volume is mounted correctly
4. Review sandbox logs for subprocess errors

### Ralph Loop Not Progressing

**Symptoms:** Status shows "running" but tasks aren't completing

**Debug steps:**

1. Check `status.json` in the job workspace for current state
2. Review `progress.txt` for iteration logs
3. Verify feedback commands are valid and executable
4. Check for CLI timeout issues
