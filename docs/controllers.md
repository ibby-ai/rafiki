...
This document explains the role and implementation of the controller module that serves as the long-lived worker for the agent sandbox system.

## Overview

The system uses a **single controller service** running in a long-lived sandbox:
...
- `session_key`: **Deprecated in controller** (mapped at Cloudflare edge to a `session_id`)
...
Session mappings are handled at the Cloudflare edge (KV cache). The controller
only consumes explicit `session_id` values and does not persist session key
lookups.
