...
|---|---|---|---|---|---|---|
| TD-001 | Post-cutover cleanup: remove remaining Phase 2 assumptions and parity debt | Platform | Open | 2026-03-31 | `docs/exec-plans/completed/phase-3-cloudflare-first/PLAN_phase-3-cloudflare-first.md` | Public traffic is already Cloudflare-first; remaining debt is limited to non-canonical migration docs that still describe rollout/fallback phases (for example `edge-control-plane/AUTH.md` and `edge-control-plane/IMPLEMENTATION_SUMMARY.md`). |
